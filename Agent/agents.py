    ...
    for json_str in json_matches:
        try:
            ...
            if "message_type" in parsed_json:
                # print("哈", json_str)
                last_json = (
                    json_str if parsed_json["message_type"] == "RTICKET" else last_json
                )
        except json.JSONDecodeError as e:
            print(f"[Agent]Json parsing ERROR: {e}\n{json_str}")
    if last_json == "":
...
class MySerial:
    ...
    def process_data(self, data):
        ...
        self.device_manager.agents[
            self.device_manager.cummunicating_agent
        ].msg_receiver._recv_xxx_message()
    ...
    def read_from_serial(self):
        ...
        while self.running:
            ...
            if self.serial_port.in_waiting:  # 有資料時讀取
                ...
                tmp_data = self.serial_port.read(1500000).decode(
                    "utf-8", errors="ignore"
                )
...
class DeviceManager:
    ...
    def remove_storage(self):
        ...
        folder_path = os.path.join(
            current_dir, "ureka_framework", "resource", "storage", "SimpleStorage"
        )
    ...
    def generate_request(self, to_agent, ticket_type, device_id="no_id"):
        ...
        elif ticket_type == "own":
            u_ticket_type = u_ticket.TYPE_OWNERSHIP_UTICKET
        elif ticket_type == "saccess":
            u_ticket_type = u_ticket.TYPE_SELFACCESS_UTICKET
        elif ticket_type == "access":
            u_ticket_type = u_ticket.TYPE_ACCESS_UTICKET
    ...
    def issue(self, from_agent: int, to_agent: int, ticket_type: str):
        ...
        request = self.generate_request(
            self.agents[to_agent],
            ticket_type,
            device_keys[-1] if device_keys else "no_id",
        )
        logger.debug(f"Request: {request}")
        if from_agent == to_agent:
            self.agents[
                from_agent
            ].flow_issuer_issue_u_ticket.issuer_issue_u_ticket_to_herself(
                device_id=request["device_id"], arbitrary_dict=request
            )
        else:
            self.agents[
                from_agent
            ].flow_issuer_issue_u_ticket.issuer_issue_u_ticket_to_holder(
                device_id=request["device_id"], arbitrary_dict=request
            )
    ...
    def holder_send_r_ticket(self, from_agent: int, to_agent: int):
        ...
        self.agents[
            from_agent
        ].flow_issuer_issue_u_ticket.holder_send_r_ticket_to_issuer(target_id)
        self.agents[to_agent].msg_receiver._recv_xxx_message()
    ...
    def apply(self, agent: int, command="HELLO-1"):
        ...
        if command == "":
            command = "HELLO-1"
        self.agents[agent].flow_apply_u_ticket.holder_apply_u_ticket(
            unuse_ticket_list[0], command
        )
    ...
    def token(self, agent: int, command="HELLO-1"):
        ...
        environment.initialize()
        self.agents[agent].flow_issue_u_token.holder_send_cmd(
            unuse_ticket_list[0], command, ("ACCESS_END" in command)
        )
        ...
        while True:
            ...
            if action == "init":
                ...
                for i in list_voter:
                    ...
                    my_manager.token(
                        1,
                        f"C-{my_manager.agents[i].shared_data.this_person.person_pub_key_str}",
                    )
                    print(
                        "config ",
                        my_manager.agents[i].shared_data.this_person.person_pub_key_str,
                        " done",
                    )
                ...
                questions = [
                    inquirer.List(
                        ...
                        choices=[
                            f"{2+i}: {my_manager.agent_name[2+i]}"
                            for i in range(config_num)
                            if "User" in my_manager.agent_name[2 + i]
                        ],
                    ),
                ]
        ...
        while True:
            questions = [
                inquirer.List(
                    ...
                    choices=[
                        f"{i}: {my_manager.agent_name[i]}"
                        for i in range(len(my_manager.agents))
                    ],
                ),
                inquirer.List(
                    "action",
